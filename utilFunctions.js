...
export async function fetchResponse(method='GET', urlPath, body='', showSpinner = true) {
    ...
    try {
        ...
        if (!response.ok) {
            if (contentType && contentType.includes('application/json')) {
                ...
                if (responseObj.message === 'Access Token has expired') {
                    ...
                    try {
                        ...
                        if (refreshErr.message === 'REFRESH_TOKEN_EXPIRED') {
                            logger.warning(`Refresh token has expired. Run 'insighta login' to login again.`)
                        } else {
                ...
                logger.error(`HTTP error! status: ${response.status} message: ${responseObj.message}`)
                return 

            } else {
                const text = await response.text()
                if (spinner) spinner.fail('Request failed')
                logger.error(`HTTP error! status: ${response.status}, received HTML instead of JSON`)
    ...
    try {
        ...
        if (!credentialsResponse.ok) {
            ...
            if (newCredentialsObj.status !== 'success') {
                ...
                throw new Error(`Refresh failed with status: ${credentialsResponse.status}`)
            }
...
export function formatTable(data) {
    ...
    const headerLines = headers.map((header, i) => wrapText(header.toUpperCase(), columnWidths[i]))
    ...
    rows.forEach((row, rowIdx) => {
        ...
        for (let lineIdx = 0; lineIdx < maxLines; lineIdx++) {
            const rowText = border.vertical + 
                wrappedCells.map((lines, i) => {
                    const color = colors[i % colors.length]
                    const text = lines[lineIdx] || ''
    ...
    const bottomBorder = border.bottomLeft + 
        columnWidths.map(w => border.horizontal.repeat(w)).join(border.bottomJoin) + 
        border.bottomRight
    console.log(chalk.gray(bottomBorder))
    
    // Print footer with row count
    console.log(chalk.gray(`${rows.length} row${rows.length !== 1 ? 's' : ''} displayed\n`))
}
    ...
    const showPage = async (page) => {
        ...
        if (response && response.data) {
            ...
            console.log(chalk.yellow('Controls: ') + 
                ...
                chalk.white('[p]') + chalk.gray(' previous  ') +
